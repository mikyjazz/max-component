...
DEVICE_SCHEMA = vol.Schema(
    {
        vol.Required(CONF_PLATFORM): "max",
...
def setup(hass, config):
    ...
    hass.bus.listen_once(EVENT_HOMEASSISTANT_STOP, hass.data[DATA_MAX].stop)
    ...
    def _service_handle_value(service):
        ...
        if not entities:
            _LOGGER.error("No Max! hubs available")
            return
    ...
    def _service_handle_reconnect(service):
        """Service to reconnect all Max! hubs."""
        homegear.reconnect()
    ...
    def _service_handle_device(service):
        """Service to call setValue method for Max! devices."""
        address = service.data.get(ATTR_ADDRESS)
        channel = service.data.get(ATTR_CHANNEL)
        param = service.data.get(ATTR_PARAM)
    ...
    def _service_handle_install_mode(service):
        ...
        homegear.setInstallMode(interface, t=time, mode=mode, address=address)
    ...
    def _service_put_paramset(service):
        ...
        homegear.putParamset(interface, address, paramset_key, paramset, rx_mode)
...
def _get_devices(hass, discovery_type, keys, interface):
    """Get the Max! devices for given discovery_type."""
    device_arr = []

    for key in keys:
        device = hass.data[DATA_MAX].devices[interface][key]
        ...
        if discovery_type == DISCOVER_BINARY_SENSORS:
...
def _device_from_servicecall(hass, service):
    """Extract Max! device from service call."""
    address = service.data.get(ATTR_ADDRESS)
    interface = service.data.get(ATTR_INTERFACE)
    ...
    if interface:
        return hass.data[DATA_MAX].devices[interface].get(address)

    for devices in hass.data[DATA_MAX].devices.values():
